...
import { _1 as nPart1, _2 as nPart2 } from "./JUDGE_SOLUTION.js"
...
const getMaxJoltage = (bank: string): number => {
  ...
  for (let i = 12; i > 0; i--) {
    ...
    const availableBatteries = batteriesWithIndeces.filter(
      (bat, i) => i >= lower && i <= upper,
    )
    const [highestNumber, index] = availableBatteries
